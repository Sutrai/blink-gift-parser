...
public class GetGemsSpecificConfig {
    ...
    public Client feignClient() {
        okhttp3.OkHttpClient delegate = new okhttp3.OkHttpClient.Builder()
                .protocols(Collections.singletonList(Protocol.HTTP_1_1)) // Только 1.1 для ротации
                .connectionPool(new ConnectionPool(0, 1, TimeUnit.NANOSECONDS)) // Разрыв сокета сразу
                .connectTimeout(60, TimeUnit.SECONDS)
                .readTimeout(120, TimeUnit.SECONDS)
                .proxy(new Proxy(Proxy.Type.HTTP, new InetSocketAddress("127.0.0.1", 8000))) // Твой прокси
                .build();
    ...
    public RequestInterceptor requestInterceptor() {
        return template -> {
            ...
            // Принудительное закрытие соединения для смены IP на прокси-сервере
